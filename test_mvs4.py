...
parser.add_argument(
    "--depth_inter_r", type=str, default="0.5,0.5,0.5,1", help="depth_intervals_ratio"
)
...
parser.add_argument(
    "--group_cor_dim", type=str, default="8,8,4,4", help="group correlation dim"
)
parser.add_argument("--inverse_depth", action="store_true", help="inverse depth")
parser.add_argument(
    "--agg_type", type=str, default="ConvBnReLU3D", help="cost regularization type"
)
parser.add_argument("--dcn", action="store_true", help="dcn")
parser.add_argument("--arch_mode", type=str, default="fpn")
parser.add_argument(
    "--ot_continous", action="store_true", help="optimal transport continous gt bin"
)
...
parser.add_argument(
    "--use_raw_train", action="store_true", help="using 1200x1600 training"
)
parser.add_argument(
    "--mono", action="store_true", help="query to build mono depth prediction and loss"
)
parser.add_argument(
    "--split", type=str, default="intermediate", help="intermediate or advanced"
)
...
def save_depth(testlist):
    ...
    gpu_measure = torch.cuda.max_memory_allocated() / 1024.0 / 1024.0 / 1024.0
        ...
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            print(
                "Iter {}/{}, Time:{} Res:{}".format(
                    batch_idx, len(TestImgLoader), end_time - start_time, imgs[0].shape
                )
            )
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(
                ...
                print(f"SAVED DEPTH TO {args.outdir}")
                ...
                if args.save_jpg:
                    for stage_idx in range(4):
                        depth_jpg_filename = os.path.join(
                            args.outdir,
                            filename.format(
                                "depth_est",
                                "{}_{}.jpg".format("stage", str(stage_idx + 1)),
                            ),
                        )
                        stage_depth = outputs["stage{}".format(stage_idx + 1)]["depth"][
                            0
                        ]
                        ...
                        if stage_idx == 0:
                            continue
                        mono_depth_jpg_filename = os.path.join(
                            args.outdir,
                            filename.format(
                                "depth_est",
                                "{}_{}.jpg".format("mono", str(stage_idx + 1)),
                            ),
                        )
                        stage_mono_depth = outputs["stage{}".format(stage_idx + 1)][
                            "mono_depth"
                        ][0]
                ...
                confidence_list = [
                    outputs["stage{}".format(i)]["photometric_confidence"].squeeze(0)
                    for i in range(1, 5)
                ]
                ...
                img = np.clip(np.transpose(img, (1, 2, 0)) * 255, 0, 255).astype(
                    np.uint8
                )
...
def mrun_rst(eval_dir, plyPath):
    ...
    os.chdir(eval_dir)
    os.system(
        "/mnt/cfs/algorithm/xiaofeng.wang/jeff/code/MVS/misc/matlab/bin/matlab -nodesktop -nosplash -r \"BaseEvalMain_func('{}'); quit\" ".format(
            plyPath
        )
    )
    print("Runing ComputeStat_func.m...")
    os.system(
        "/mnt/cfs/algorithm/xiaofeng.wang/jeff/code/MVS/misc/matlab/bin/matlab -nodesktop -nosplash -r \"ComputeStat_func('{}'); quit\" ".format(
            plyPath
        )
    )
...
if __name__ == "__main__":
    ...
    if args.dataset.startswith("general"):
        ...
        mrun_rst(eval_dir="./evaluations/dtu/", plyPath="./" + args.outdir[1:])
